...
\begin{tikzpicture}[scale=0.7,auto,semithick,>=to,decoration={markings,mark=at position 0.5 with {\arrow{>}}}]
	...
	\pgftransformrotate{\dropletOrient}
		...
		\draw[gray!50] (0cm, 0cm) -- (2.2cm,0cm);
%		\draw[] (-120:\motDist) -- (60:\legDist) node[below, midway] {\large$\ell$}; %This is the line showing '\ell'
		\foreach \i in {1,...,3} {
			\node[rectangle,draw=black,fill=\motShade,minimum width=\motWidth, minimum height=\motHeight, 
						rotate around={(-\i*120-240+\dropletOrient):(0,0)},label={[label distance=-0.05cm]-180:$m_\i$}] at (-120*\i-240:\motDist) {};
			\node[circle, draw=none, fill=\legShade, inner sep=\legRad,rotate around={-\i*120-60:(0,0)},label={[label distance=-0.13cm]90:{\small$l_\i$}}] at (-60-120*\i:\legDist) {};
		}
