...
(X-AXIS GOES OUT TO THE RIGHT, Y-AXIS GOES UP, THETA IS POSITIVE IN COUNTER CLOCKWISE NEED TO ADD TO DRAWING)
\begin{eqnarray}
\dot{x}_r &=&\\
\dot{y}_r &=& \theta_1 - \theta_2\\
\dot{\theta}_r &=& \theta_0 + \theta_1 + \theta_2
\end{eqnarray}
Here, we assume positive $\theta_i$ to be aligned with the direction of thrust of motor $m_i$. That is, if all motors spin in the same direction, they all contribute to $\dot{\theta}_r$ in the same way.  
